...
class TestCourseContext(TestCase):
    ...
    def test_get_name_email_info(self):
        """
        Happy path test for a known contact
        """
        self.context.query = "Mike Rogers"
        self.context.get_directory_info()
        self.assertEqual("mrodgers@tntech.edu", self.context.entries[0].email)
        print(self.context)

    def test_get_building(self):
        self.context.query = " Prescott 215"
        self.context.get_directory_info()
        self.assertEqual("Prescott 215", self.context.entries[0].building)
        print(self.context)
    def test_get_firstname(self):
        self.context.query = "Mike"
        self.context.get_directory_info()
        self.assertEqual("mrodgers@tntech.edu", self.context.entries[0].firstname)
        print(self.context)
    def test_get_lastname(self):
        self.context.get.query = "Rodgers"
        self.context.get_directory_info()
        self.assertEqual("Rodgers", self.context.entries[0].lastname)
        print(self.context)
